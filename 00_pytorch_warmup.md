这里为你重新整理了一份纯中文、结构清晰、专业规范的 Markdown 技术笔记。你可以直接将其保存为 `README.md` 或相关的博客文章上传到 GitHub。

这份笔记不仅包含完整的代码，还融合了我们前面讨论过的 **PyTorch 维度设计哲学** 以及 **反向传播梯度的详细推导**，非常适合作为技术积累。
...
已知从上一层传回的梯度为 $\text{grad\_output} = \frac{\partial L}{\partial y}$，我们通过**链式法则**依次推导各个部分的梯度：
